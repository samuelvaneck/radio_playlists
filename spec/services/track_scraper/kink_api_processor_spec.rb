...
describe TrackScraper::KinkApiProcessor, type: :service do
  describe '#last_played_song' do
    subject(:last_played_song) { kink_api_processor.last_played_song }
    ...
    before do
      allow(kink_api_processor).to receive(:make_request).and_return(api_response)
    end
    ...
    context 'when the API response is valid' do
      ...
      it 'sets the artist name' do
        last_played_song
        expect(kink_api_processor.instance_variable_get(:@artist_name)).to eq('Test Artist')
      end

      it 'sets the title' do
        last_played_song
        expect(kink_api_processor.instance_variable_get(:@title)).to eq('Test Title')
      end

      it 'sets the broadcasted_at timestamp' do
        last_played_song
        expect(kink_api_processor.instance_variable_get(:@broadcasted_at)).to be_within(1.second).of(Time.zone.now)
      end
    ...
    context 'when the API response is blank' do
      ...
      it 'logs the error' do
        last_played_song
        expect(Rails.logger).to have_received(:warn).with(/KinkApiProcessor:/)
      end
    end
  end
end
