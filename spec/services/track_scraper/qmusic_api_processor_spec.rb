  ...
  describe '#last_played_song' do
    ...
    context 'when the API response is valid' do
      ...
      it 'sets the broadcasted_at timestamp' do
        last_played_song
        expect(qmusic_api_processor.broadcasted_at).to eq(Time.find_zone('Amsterdam').parse('2025-08-05T11:00:47+02:00'))
      end

      it 'sets the artist name' do
        last_played_song
        expect(qmusic_api_processor.artist_name).to eq('Ed Sheeran')
      end

      it 'sets the title' do
        last_played_song
        expect(qmusic_api_processor.title).to eq('Sapphire')
      end

      it 'sets the Spotify URL' do
        last_played_song
        expect(qmusic_api_processor.spotify_url).to eq('https://open.spotify.com/track/4Q0qVhFQa7j6jRKzo3HDmP')
      end

      it 'sets the YouTube ID' do
        last_played_song
        expect(qmusic_api_processor.youtube_id).to eq('JgDNFQ2RaLQ')
      end

      it 'sets the website URL' do
        last_played_song
        expect(qmusic_api_processor.website_url).to eq('http://edsheeran.com/')
      end

      it 'sets the Instagram URL' do
        last_played_song
        expect(qmusic_api_processor.instagram_url).to eq('https://instagram.com/teddysphotos')
      end
