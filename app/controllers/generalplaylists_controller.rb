class GeneralplaylistsController < ApplicationController
  ...
  def today_played_songs
    @todayplayedsongs = Generalplaylist.today_played_songs.paginate(page: params[:page]).per_page(10)
  end

  def top_songs
    @topsongs = Generalplaylist.top_songs.paginate(page: params[:page]).per_page(10)
    @counter = 0
  end
