...
- `LyricsThemeBreakdownCalculator` - Top 10 lyric themes for a station's airplays in a period. Aggregates `lyrics.themes` via PostgreSQL `unnest`; share is fraction of plays-with-themes containing the theme (can exceed 1.0 across rows since plays carry multiple themes)
